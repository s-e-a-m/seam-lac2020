...
\begin{description}
  \item[seam.lib] contains general functions and the pointers to each specific libraries. It may contain the custom performative environment definition, as for the inputs, the controls and outputs.
...
The hardware and software development between the two systems was oriented on real-time musical interaction as, during those years, was pioneered by the major research labs. At the same CIM\footnote{\url{http://www.aimi-musica.org/?page_id=13}} event in which Lupone describe the System Fly \cite{ml85},  Sylviane Sapir and Alvise Vidolin \cite{savi85} describe with inspiring words the real-time Prometeo production

\begin{quote}
Con l'avvento dei sistemi informatici in tempo reale e diventato possibile ri-mettere in diretta corrispondenza il gesto con il suono ricreando il feed-back azione-suono-ascolto-azione e, di conseguenza, far uscire gli elaboratori dai laboratori e concepire delle composizioni che prevedessero una esecuzione dal vivo sia autonoma sia integrata con altri strumenti tradizionali e non\footnote{With the appearance of real-time informatics systems, it has become possible to re-put the gesture and the sound in direct correspondence, recreating the action-sound-listening-action feed-back and, consequently, bringing the computers out of the laboratories and conceiving compositions that provided for a live performance both autonomous and integrated with other traditional and non-traditional instruments.}. 
\end{quote}
...
Figure \ref{ml-fly10} shows as Fly 10 modules are connected to the host interface at one side, and the DAC at other. Up to two parallel systems, made up of a card that allows the signal processing, a 12-bit DAC converter, a Butterworth Low Pass Filter made by a series of four cells of the 2nd order with two selectable cut-off frequencies at 4.5 Khz or 9.3 Khz.

Knowing the structure of \emph{Fly 10}, and its procedure to generate sound at the electric stadium, is fundamental to understanding some composer tuning of algorithms and to balance the new real-time processing made by the porting and some of the original tape sounds made with the \emph{Fly 10} system. The 1991 state of the art of the \emph{System Fly 30} structure had 16Bit ADC/DAC with a sample rate up to 96KHz. For the release of \emph{Mobile Locale}, the label Edipan mastered the tape with upsampling to 16Bit at 48000Hz. The sounds generated with today real-time architecture could be in high resolution and higher sampling frequencies. Discussing with Lupone of all those historical data, and the new procedure, underlined the necessity to have a set of filters, similar to the one at the end of \emph{Fly} systems, to search a timbral balancing during the staging. 

The entire electronics, both tape and live, was conceived as a shadow of the acoustic percussions and amplification of them. So there are three levels of musical matter on stage: the acoustic (complex set of percussions), the electroacoustic(sound reinforcement and early reflections simulation), the live electronics (real-time processes based to three different delay line usages). The balancing of electronics with the electroacoustic must point to hide the electronics inside the percussion sounds\footnote{Personal conversation with the composer.}.

There are four places, areas, described in the algorithm respectively the QAQF Delay line with oscillator reading index, the comb filtering  Early Reflections, the scaled mix of the reflection and the two feedback delay lines WA and ZA.

The first real-time object, named \emph{QAQF}, as in figure \ref{ml-gen-dia}, is a delay line read by an oscillator at some fixed rates. The higher of them, 320Hz, produces a broad signal, full of digital artefacts, that could assume different meaning if the output signal is not treated by out filters stage.
